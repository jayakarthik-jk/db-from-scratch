use std::{fs::File, io::{self, Read, Seek, Write}, path::Path};

use crate::backend::page::PAGE_SIZE;

use super::{page::Page, row::{ROWS_PER_PAGE, ROW_SIZE}};
...
impl Pager {
    ...
    pub(crate) fn flush(&mut self, rows: usize) {
        ...
        for (i, page) in self.pages.iter().enumerate() {
            ...
            let rows = if i < full_pages { ROW_SIZE } else { rows % ROWS_PER_PAGE };
            // flush page
            self.file
